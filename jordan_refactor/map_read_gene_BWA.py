...
# - changed the gene_map() function to deal with thresholds and
...
def gene_map(sam, unmapped):
    ...
    with open(sam,"r") as samfile:
        for line in samfile:
            ...
            # does query meet alignment threshold?:
            length= 0
            matched= 0
            ...
            query2gene_map[query].add(db_match)             # Collected all aligned genes for contig/read.
    ...
    for query, genes in list(query2gene_map.items()):   # Check all queries to see
        if len(genes)>1:                                #  is they are aligend to multiple genes, and if so
    ...
    # alignment is recorded, while the other alignments fail the "on-the-fly" filter.
    # This is avoided between the unmerged paired reads, because paired reads only have one
    # BWA output file and are therefore analyzed once (together).
...
# Extract BWA output:
# loop over readtype sets: contigs, merged, unmerged1, unmerged2
for x in range((len(sys.argv)-4)/3):
    ...
    if x!=3:                                        # Only do once for unmerged paired end (same .sam file).
        unmapped_reads= set()                       # Initialize set of unmapped readIDs.
        gene_map(BWA_sam_file, unmapped_reads)      # BWA-aligned contigs/reads are stored in gene2read_map
...
print '\nReads mapped to ' + str(len(genes)) + ' genes.'