...
gene_seqs = SeqIO.index(gene_file, "fasta")
prot_file = sys.argv[5]
...
for x in range((len(sys.argv) - 6) / 3):
    read_file = sys.argv[3 * x + 6]
    read_seqs = SeqIO.index(read_file, os.path.splitext(read_file)[1][1:])
    DMND_tab_file = sys.argv[3 * x + 7]
    output_file = sys.argv[3 * x + 8]
...
with open(prot_file, "w") as out_prot:
    SeqIO.write(genes_trans, out_prot, "fasta")
