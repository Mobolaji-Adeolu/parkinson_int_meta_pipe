...
with open(gene2read_file,"r") as mapping:
    for line in mapping:
        if len(line)>5:                             # line at least 5 characeters?
            ...
            gene_len[entry[0]]= entry[1]            # key=geneID, value=gene length
...
for x in range((len(sys.argv)-6)/3):
    ...
    def gene_map(tsv,unmapped):
        ...
        for line in Sorted_Hits:
            ...
            align_len= 3*line[3]            # alignment length (aa -> nt)
            score= line[11]                 # score
            ...
            if float(seq_identity) > float(identity_cutoff):                    # identity
                ...
                if count4%1000 == 0:
                    ...
                    print 'float(align_len)= ' + str(float(align_len))
                ...
                if float(align_len) > len(read_seqs[query].seq)*length_cutoff:  # length
                    ...
                    if float(score) > float(score_cutoff):                      # score
                        ...
                        # ***could save time by just looking at DMD-aligned
                        # proteins, instead of full gene2read_map which
                        # includes BWA&BLAT-aligned genes***
                        if db_match in gene2read_map:                           # If query matches to a previously found prot:
                            if contig:                                          # If query is a contig, then
                                ...
                                for read in contig2read_map[query]:             #  take all reads making up that contig
                                    if read not in gene2read_map[db_match]:     #  not already assigned to that matched prot (by BWA/BLAT/DMD)
                                        if read not in mapped_reads:            #  and not already assigned by DMD to a diff prot:
                                            gene2read_map[db_match].append(read)#  and append their readIDs to aligned gene/prot<->read dict,
                                            mapped_reads.add(read)              #  and mark them as assigned by DMD.
                            elif not contig:                                    # If query is a read, and
                                if query not in gene2read_map[db_match]:        #  it hasn't already been assigned to that matched prot:
                                    gene2read_map[db_match].append(query)       #  append its readID to aligned gene/prot<->read dict,
                                    mapped_reads.add(query)                     #  and mark it as assigned by DMD.
                    
                        # query matches to BWA&BLAT&DMD-novel prot:
                            ...
                            if contig:                                          # If query is a contig, then
                                ...
                                for read in contig2read_map[query]:             #  take all reads making up that contig
                                    if read not in mapped_reads:                #  not already assigned by DMD to a diff prot:
                                        ...
                                        read_count+= 1
                                        if read_count==1:
                                            gene2read_map[db_match]= [read]     #  add its readID to aligned gene/prot<->read dict
                                        elif read_count>1:
                                            gene2read_map[db_match].append(read)#  or append its readID to aligned gene/prot<->read dict.
                            elif not contig:                                    # If query is a read, then
                                gene2read_map[db_match]= [query]                #  add its readID to aligned gene/prot<->read dict
                                mapped_reads.add(query)                         #  and mark it as assigned by DMD.
    ...
    #  Novel reads that meet threshold, append to gene2read_map (aligned gene/protID<->readID(s) dict).
...
with open(gene2read_file,"w") as out_map:               # Delete old gene2read_file and write a new one.
    ...
    # write genes:
    for gene in gene2read_map:                          # Take each BWA&BLAT&DMD-aligned gene/prot and
        try:
            out_map.write(gene + "\t" + gene_len[gene] + "\t" + str(len(gene2read_map[gene])))
                                                        #  write [aligned gene/protID, length (in nt), #reads],
                                                        #  gene_len[gene] WILL FAIL FOR DIAMOND ALIGNED, not defined
    ...
    for record in SeqIO.parse(Prot_DB,"fasta"):         # Loop through SeqRec of all prot in PROTdb:
        ...
        if record.id in gene2read_map:                  #  If PROTdb prot is one of the matched proteins,
