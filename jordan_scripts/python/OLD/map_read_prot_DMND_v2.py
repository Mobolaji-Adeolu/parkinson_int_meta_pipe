...
def gene_map(tsv, unmapped, seq_len):
    ...
    for line in Sorted_Hits:
        ...
        if seq_identity > identity_cutoff:                  # identity
            if align_len > seq_len[query]*length_cutoff:    # length
...
for x in range((len(sys.argv)-6)/3):
    ...
    read_seqs_len= {}
    for k in read_seqs:                                 # Create dict of non-BWA-aligned read lengths:
        read_seqs_len[k]= len(read_seqs[query].seq)     #  contig/readID<->length (amino acids?).
    
    # for DMD-aligned contigs/reads that
    # don't meet threshold:
    unmapped_reads= set()                               # Initialize set of contig/readIDs.
    unmapped_seqs= []                                   # Initialize list of SeqRecords.

    # process DMD-aligned reads:
    gene_map(DMND_tab_file, unmapped_reads, read_seqs_len)
